...
- 남의 파일 해시를 주장해 그 내용을 가져가는 것 — 업로드 시 서버가 본문을 직접
  해싱해 `hash_id` 와 대조한다. 불일치면 400
- 감사 로그 IP 위조 — nginx 가 채우는 `X-Real-IP` 를 쓰고, `X-Forwarded-For` 는
  클라이언트가 앞에 값을 끼워 넣을 수 있으므로 마지막 항목만 신뢰한다
