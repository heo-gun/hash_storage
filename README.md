...
**VFS + 콘텐츠 그래프 시각화**
VFS 자체는 사용자별 트리다 — `fs_nodes`의 `parent_id`는 하나뿐이고, 중복 파일은
**여러 `fs_node` 행이 같은 `hash_id`를 가리키는** 형태로 존재한다. `/app`의 Graph
탭은 같은 `hash_id`를 노드 하나로 병합해 그리므로, 여러 경로가 동일 콘텐츠를
참조하는 구조가 DAG 형태로 보인다. 병합은 시각화 단계에서 일어나며 저장 구조는
트리 그대로다.
...
같은 파일을 올려도 S3에는 하나만 존재하며, 서로의 트리에는 접근할 수 없다. 다만 업로드
응답의 dedup 여부가 "그 내용이 시스템에 이미 있다"는 사실은 드러낸다 — 전역 dedup에
따라오는 존재 오라클이다.
...
> 접근 자격 증명은 256비트 `share_token` 이다(수신자 계정 불필요). 이메일을 지정하면
> 그 주소로 로그인해야 열린다. `.epf` 암호문은 저장하지 않고 요청 시점에 만든다 —
> S3 원본 자체의 보호는 IAM·버킷 정책에 의존한다.
...
- **2단계 업로드** — 지금은 dedup이 확실한 경우에도 본문을 전송한다. 해시를 먼저
  조회하고 miss일 때만 올리면 전송량을 아낄 수 있지만, **본문 없이 blob을 참조하게
  하는 것이 곧 content-confirmation 공격 벡터다.** 도입하려면 소유 증명(서버가 임의
  구간의 해시를 요구)이 함께 있어야 한다.
